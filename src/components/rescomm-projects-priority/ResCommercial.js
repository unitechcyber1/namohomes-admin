...
function ResCommercial() {
  ...
  const handleFetchTopProjects = async (cityId) => {
    ...
    const data = await getTopProjectsByCity(query, cityId, url)
  ...
  const onChangeOptionHandler = (selectedOption, dropdownIdentifier) => {
    switch (dropdownIdentifier) {
      ...
      query.project_type = selectedOption ? selectedOption.label : null
      getProjectDataWithPagination()
  ...
  useEffect(() => {
   ...
   setQuery({ name: "", city: "", location: "", status: "approve", page: 1, limit: 10, project_type: "" })
   handleFetchCity();
  }, [url]);
  ...
  const handleCheckboxChange = async (event, project) => {
    ...
    try {
      ...
      project.rescomm_priority.is_active = checked;
                          ...
                          <Td colSpan={8} textAlign="center">
                            ...
                            <Tr key={project._id}>
                              <Td>
                                <input
                                  type="checkbox"
                                  checked={project.rescomm_priority.is_active}
                                  onChange={(event) =>
