...
function TopBuilders() {
  ...
  const [query, setQuery] = useState({ name: "", page: 1, limit: 10 })
  const url = window.location.href;
  const getProjectDataWithPagination = async () => {
    setLoading(true)
    const data = await getAllbuildersData(query)
    setprojects(data.data)
    setTotalCount(data.totalCount)
    setLoading(false)
  }
  const handleFetchTopProjects = async () => {
    setLoadingTable(true)
    const data = await getTopBuilders()
  ...
  const handleCheckboxChange = async (event, project) => {
    ...
    try {
      ...
      await changeOrderOfBuilder(updatedproject, project._id)
    ...
    <div className="mx-5 mt-3">
      ...
      <div className="table_container">
        ...
        <div className="table-box top_table_box2">
          <div className="table-top-box">Popular Builders Module</div>
          <TableContainer style={{ overflowX: "hidden" }}>
            <div className="data_table">
              <div className="row">
                <div className="col-md-12">
